...
x_min_before=0.30
x_min_after=0.40
x= npy.linspace(x_min_before,x_min_after,5)
epsilon_2=7700
Rratio=npy.zeros(len(x))

for i in range(0,len(x)):

	Rratio[i]=Rrat(P_atm,Tg_atm,Rg_atm,M,x=x[i],epsilon_2=epsilon_2,Vratio=Vratio,Pstar=Pstar,Tstar=Tstar,Rstar=Rstar)
...
for j in range(0,len(x)):
	for i in range(0,len(P)):
		Tg_calculated[i]=glassTemp(P[i],M=M,x=x[j],Rratio=Rratio[j],Tratio=Tratio,Vratio=Vratio,Pstar=Pstar,Tstar=Tstar,Rstar=Rstar)
		print P[i]
